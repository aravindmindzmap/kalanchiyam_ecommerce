...
class CartController extends Controller
{
    ...
    public function getVariantPrice(Request $request): array
    {
        ...
        foreach ($getCartList as $cartItem) {
            if ($cartItem['product_id'] == $product['id'] && $cartItem['variant'] == $string) {
                // dd($cartItem['variant'],$string,$productVariationCode);
                $inCartExistStatus = 1;
                $inCartExistKey = $cartItem['id'];

                if (Str::contains(url()->previous(), '/product/')) {
                    $requestQuantity = $request['quantity'];
                    if (empty($request['key'])) {
                        $requestQuantity = $cartItem['quantity'];
                    }
                }else{
                    $requestQuantity = ($request['quantity'] > $cartItem['quantity'] && $productVariationCode === $string) ? $request['quantity']: (($request['quantity'] < $cartItem['quantity']) ? $cartItem['quantity'] : $request['quantity']);
                }
        ...
        return [
            ...
            // 'in_cart_current_quantity' => $cartCurrentQuantity,
    ...
    public function updateQuantity_guest(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'in_cart_quantity' => $response['status'] == 0 ? $response['qty'] : $request->quantity,
        ]);
    }
