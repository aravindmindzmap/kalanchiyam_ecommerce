    ...
    <div class="product__summary d-flex flex-column align-items-center gap-1 pb-3 cursor-pointer">
        ...
        @else
            <div class="d-flex justify-content-between align-items-center gap-10 add-to-cart-section"
                ...
                <button class="btn btn-primary add-to-cart-enable-btn product-add-to-cart-button-in-product-cart dynamicSectionAbc{{ $in_cart_key?->id ?? 0 }}  {{ isset($in_cart_key) ? 'hide' : 'show' }} "
                ...
                <div class="add-to-cart-box {{ isset($in_cart_key?->id) ? 'show' : 'hide' }} dynamicSectionXyz{{ 'Cart'.$in_cart_key?->id ?? 'Product'.$product?->id }}">
                ...
                <div class="qty-input-box {{ isset($in_cart_key?->id) ? 'show' : 'hide' }} dynamicSectionXyz{{ 'Cart'.$in_cart_key?->id ?? 'Product'.$product?->id }}">
...
<script>
    ...
    function qty_count_add(e) {
        ...
        if (maximum_order_qty <= current_val) {
            add_product_qty.val(current_val);
        } else {
            add_product_qty.val(parseInt(current_val) + 1);
            $(qty_count_add).closest(".add-to-cart-section").find('.product-add-to-cart-button-in-product-cart').data(
                'quantity', parseInt(current_val) + 1);
        }
    ...
    function qty_count_minus(e) {
        ...
        if (minimum_order_qty >= current_val) {
            add_product_qty.val(current_val);
            // $(qty_count_minus).parent('.qty-input-box').toggleClass("hide show");
            // $(qty_count_minus).parent('.qty-input-box').siblings('.add-to-cart-box').toggleClass("hide show");
            // $(qty_count_minus).parent('.qty-input-box').siblings('.add-to-cart-enable-btn').toggleClass("hide show");
        } else {
            add_product_qty.val(parseInt(current_val) - 1);
            $(qty_count_minus).closest(".add-to-cart-section").find('.product-add-to-cart-button-in-product-cart').data(
                'quantity', parseInt(current_val) - 1);
    ...
    function add_to_cart_enable_btn(e) {
        ...
        const add_to_cart_enable_btn_siblings = $(add_to_cart_enable_btn).siblings();

        $(add_to_cart_enable_btn_siblings).toggleClass("hide show");
        $(add_to_cart_enable_btn).toggleClass("hide show");
    }
</script>
