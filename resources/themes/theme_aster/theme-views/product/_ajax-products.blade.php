@if (count($products) > 0)
    <div class="auto-col gap-3 mobile_two_items minWidth-13rem {{ session()->get('product_view_style') == 'list-view' ? 'product-list-view' : '' }}"
        id="filtered-products" style="{{ count($products) > 4 ? '--maxWidth:1fr' : '--maxWidth:14rem' }}">
        @foreach ($products as $product)
            @include('theme-views.partials._product-small-card', [
                'product' => $product,
                'slider' => 'productsListPage',
            ])
        @endforeach
    </div>
@else
    <div class="d-flex flex-column justify-content-center align-items-center gap-2 py-5 my-5 w-100">
        <img width="80" class="mb-3" src="{{ theme_asset('assets/img/empty-state/empty-product.svg') }}"
            alt="">
                ...
                <li class="page-item {{ $page == $i ? 'active' : '' }}">
                    <label class="m-0">
                        <input type="radio" name="page" value="{{ $i }}"
                            {{ $page == $i ? 'checked' : '' }} class="real-time-action-update">
