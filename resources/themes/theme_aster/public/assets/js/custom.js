...
function addToCartOnclick() {
    ...
    // in new add to cart
    $(document).on('click', '.product-add-to-cart-button-in-product-cart', function() {

        let action = $(this).data('action');
        let token = $(this).data('_token');
        let id = $(this).data('id');
        let quantity = $(this).data('quantity');
        let product_variation_code = $(this).data('product_variation_code');
        let key = $(this).data('key');
        let type = $(this).data('type');
        let buy_now = $(this).data('buy_now');
        ...
        $form.append($('<input>', { type: 'hidden', name: 'product_variation_code', value: product_variation_code, class : 'product-generated-variation-code' }));
        $form.append($('<input>', { type: 'hidden', name: 'key', value: key , class : 'product-exist-in-cart-list'}));
        $form.append($('<input>', { type: 'hidden', name: 'buy_now', value: buy_now }));
        $form.append($('<input>', { type: 'hidden', name: 'type', value: type }));

        addToCartForAddToCartCart($form,false,null,$(this));
    });
}
...
function addToCartForAddToCartCart(formSelector, redirectToCheckout = false, url = null, atc_btn = null) {
    ...
    if (checkValidityForVariantPrice(formSelector)) {
        ...
        $.post({
            ...
            success: function (response) {
                ...
                // console.log(atc_btn);
                let in_cart_key = response.in_cart_key;
                // let value_assign_add_to_cart_box = $('.add-to-cart-box').find('.product-add-to-cart-button-in-product-cart');
                let value_assign_add_to_cart_box = $(atc_btn).siblings('.add-to-cart-box').find('.product-add-to-cart-button-in-product-cart');
                value_assign_add_to_cart_box.data('key',in_cart_key);

                let cartType = $(formSelector).find('[name="type"]').val();
                let cartProductId = response?.cart?.product_id;

                let typeId = cartType+cartProductId;

                $('.dynamicSectionAbc'+typeId).data('type','Cart');
                $('.dynamicSectionAbc'+typeId).data('cart_id',in_cart_key);
                $('.dynamicSectionAbc'+typeId).removeClass('dynamicSectionAbc'+typeId).addClass('dynamicSectionAbcCart'+in_cart_key);

                // $(atc_btn).siblings('.add-to-cart-box').removeClass('dynamicSectionXyz0').addClass('dynamicSectionXyz' + in_cart_key);
                // $(atc_btn).siblings('.qty-input-box').removeClass('dynamicSectionXyz0').addClass('dynamicSectionXyz' + in_cart_key);

                // this is new modify
                $('.dynamicSectionXyz'+typeId).data('type','Cart');
                $('.dynamicSectionXyz'+typeId).data('cart_id',in_cart_key);
                $('.dynamicSectionXyz'+typeId).removeClass('dynamicSectionXyz'+typeId).addClass('dynamicSectionXyzCart'+in_cart_key);
                $('.dynamicSectionXyz'+typeId).removeClass('dynamicSectionXyz'+typeId).addClass('dynamicSectionXyzCart'+in_cart_key);
                ...
                if (response.status === 1) {
                    ...
                    if (response.in_cart_key) {
                        $(atc_btn).data('key',response.in_cart_key);
                    }
...
function updateCartQuantity(cartId, productId, action, event) {
    ...
    if (cartQuantity.val() > cartQuantity.data("current-stock")) {
        cartItemRemoveFunction(removeUrl, token, cartId, segment, productId);
        return false;
    }
    ...
    if (
        ...
        $('.add-to-cart-details-form').find(".product-add-to-cart-button").html('Add To Cart');
        cartItemRemoveFunction(removeUrl, token, cartId, segment, productId);
    } else {
        if (cartQuantity.val() < cartQuantity.data("min")) {
            ...
            $.post(
                ...
                function (response) {
                    ...
                    var sss = $(`.cart_quantity_${cartId}`);

                    // new zone
                    let getDynamicSectionXyz = $('.dynamicSectionXyzCart'+cartId);
                    getDynamicSectionXyz.find('input').val(response["qty"]);
                    $('.add-to-cart-details-form').find(".product_quantity__qty").val(response.in_cart_quantity);
                    getVariantPrice('.add-to-cart-details-form');
...
function cartItemRemoveFunction(removeUrl, token, cartId, segment, productId) {
    ...
    $.post(
        ...
        function (response) {
            ...
            let dynamicSectionAbc = $('.dynamicSectionAbcCart'+cartId);
            let get_abc_original_quantity = dynamicSectionAbc.data('original_quantity');
            dynamicSectionAbc.data('quantity',get_abc_original_quantity);
            dynamicSectionAbc.data('key',"");
            dynamicSectionAbc.data('type',"Product");
            dynamicSectionAbc.data('cart_id',cartId);
            dynamicSectionAbc.find('input').val(get_abc_original_quantity);
            dynamicSectionAbc.removeClass('dynamicSectionAbcCart'+cartId).addClass('dynamicSectionAbcProduct'+productId);
            dynamicSectionAbc.toggleClass("hide show");

            let dynamicSectionXyz = $('.dynamicSectionXyzCart'+cartId);
            let get_xyz_original_quantity = dynamicSectionXyz.find('.product-add-to-cart-button-in-product-cart').data('original_quantity');
            dynamicSectionXyz.find('.product-add-to-cart-button-in-product-cart').data('quantity',get_xyz_original_quantity);
            dynamicSectionXyz.find('.product-add-to-cart-button-in-product-cart').data('key',"");
            dynamicSectionXyz.data('type',"Product");
            dynamicSectionXyz.data('cart_id',cartId);
            dynamicSectionXyz.find('.add-product-qty').val(get_xyz_original_quantity);
            dynamicSectionXyz.removeClass('dynamicSectionXyzCart'+cartId).addClass('dynamicSectionXyzProduct'+productId);
            dynamicSectionXyz.toggleClass("hide show");
...
function updateProductDetailsTopSection(formSelector, response) {
    ...
    $(formSelector).find(".product_quantity__qty").val(response.in_cart_quantity);
...
function actionRequestForProductRestockFunctionality() {
    ...
    $(".product-restock-request-button-in-product-cart").on("click", function () {
        let isLoggedIn = $(this).data('auth')?.toString();
        if (isLoggedIn === 'true' || isLoggedIn === '1') {
            ...
            let productCartForm = {
                ...
                'product_variation_code' : $(this).data('product_variation_code'),
                'key' : $(this).data('key')
            };

            getRequestForProductRestockForAddToCart(productCartForm,$(this));
        } else {
            toastr.warning($(".login-warning").data("login-warning-message"));
            $("#quickViewModal").modal("hide");
            $("#loginModal").modal("show");
...
function getRequestForProductRestockForAddToCart(formElement,atc_btn) {
